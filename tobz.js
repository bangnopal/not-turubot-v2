...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const cmdDB = JSON.parse(fs.readFileSync('./lib/database/commands.json'))
        
        for (let i = 0; i < cmdDB.length; i ++) {
            if (chats == cmdDB[i].pesan) {
                tobz.reply(cmdDB[i].balasan)
            }
        }
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        case prefix+'blackpink':
            ...
            await tobz.sendFileFromUrl(from, `https://turuapi.org/textmaker/blackpink?text=${blpk}&apikey=${turukey}`, 'blackpink.jpg', '', id)
            break
        case prefix+'thunder':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            ...
            await tobz.sendFileFromUrl(from, `https://turuapi.org/textmaker/thunder?text=${thndr}&apikey=${turukey}`, 'thndr.jpg', '', id)
            break
        case prefix+'pornhub':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            ...
            if (argz.length >= 2) {
                ...
                tobz.sendFileFromUrl(from, `https://turuapi.org/textmaker/pornhub?text1=${lpornhub}&text2=${lpornhub2}&apikey=${turukey}`)
        ...
        case prefix+'glitch':
            ...
            if (argz.length >= 2) {
                ...
                tobz.sendFileFromUrl(from, `https://turuapi.org/textmaker/glitch?text1=${glitch1}&text2=${glitch2}&apikey=${turukey}`)
                await limitAdd(serial)
            } else {
...
${desc}`)
        ...
        case prefix+'rate':
            ...
            const awr = GenerateRandomNumber(0, 100)
        ...
        case prefix+'randomhentai':
            ...
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            tobz.sendFileFromUrl(from, `https://turuapi.org/anime/randomhentai?apikey=${turukey}`, 'randomhentai.jpg', '', id)
            await limitAdd(serial)
            break
        case prefix+'randomnsfwneko':
            ...
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            tobz.sendFileFromUrl(from, `https://turuapi.org/anime/randomnsfw?apikey=${turukey}`, 'nsfw.jpg', '', id)
            await limitAdd(serial)
            break
        ...
        case prefix+'memeindo':
            ...
            if (cekumur(cekage)) return
            if (isLimit(serial)) return
            const memeindo = await axios.get(`https://turuapi.org/tools/memeindo?apikey=${turukey}`).then(result => {
                tobz.sendFileFromUrl(from, result.data.result.meme, 'meme.jpg', '', id)
            }).catch(e => {
                return
            })
            await limitAdd(serial)
...
🌐️ *URL*: ${url}`
        ...
        case prefix+'ytsearch':
            ...
            tobz.reply(from, mess.wait, id)
            try {
                const response2 = await fetch(`https://turuapi.org/dl/yts?query=${encodeURIComponent(ytsher)}&apikey=${turukey}`)
                if (!response2.ok) throw new Error(`unexpected response ${response2.statusText}`)
                const jsonserc = await response2.json()
                const { result } = await jsonserc
                let xixixi = `*「 YOUTUBE SEARCH 」*\n\n*Hasil Pencarian : ${ytsher}*\n`
                for (let i = 0; i < result.length; i++) {
                    xixixi += `\n─────────────────\n\n• *Judul* : ${result[i].title}\n• *Ditonton* : ${result[i].views}\n• *Durasi* : ${result[i].duration}\n• *Channel* : ${result[i].channel}\n• *URL* : https://youtu.be/${result[i].id}\n`
                }
        ...
        case prefix+'infogempa':
            ...
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const bmkg = await axios.get('https://turuapi.org/tools/infogempa?apikey=' + turukey)
	...
	case prefix+'bitly':
        ...
        case prefix+'tinyurl':
            ...
            const tiny1 = await axios.get('https://turuapi.org/tools/tinyurl?apikey=' + turukey + '&url=' + shorturl2)
            const tiny2 = tiny1.data
            if (tiny2.error) return tobz.reply(from, tiny2.error, id)
            const surl3 = `Link : ${shorturl2}\nShort URL : ${tiny2.result.shortUrl}`
        ...
        case prefix+'ytmp4':
            ...
            try {
                tobz.reply(from, mess.wait, id)
                const ytvh = await fetch(`https://turuapi.org/dl/youtube?url=${args[1]}&apikey=${turukey}`)
                ...
                const ytvh2 = await ytvh.json()
                 if (ytvh2.status == false) {
                    ...
                    if (Number(ytvh2.result.video.size.split(' MB')[0]) > 30.00) return tobz.sendFileFromUrl(from, ytvh2.result.thumbnail, 'thumb.jpg', `*「 YOUTUBE MP4 」*\n\n• *Judul* : ${ytvh2.result.title}\n• *Filesize* : ${ytvh2.result.video.size}\n\n__Maaf, Ukuran video melebihi 30 MB. Silahkan download video melalui link dibawah_.\n${ytvh2.result.video.url}`, id)
                    // const { title, UrlVideo, imgUrl, size, status, ext } = await ytvh2.result
                    // console.log(`VHTEAR : ${ext}\n${size}\n${status}`)
                    tobz.sendFileFromUrl(from, ytvh2.result.thumbnail, 'thumb.jpg', `*「 YOUTUBE MP4 」*\n\n• *Judul* : ${ytvh2.result.title}\n• *Filesize* : ${ytvh2.result.video.size}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`, id)
                    await tobz.sendFileFromUrl(from, ytvh2.result.video.url, `${ytvh2.result.title}.mp4`, '', id).catch(() => tobz.reply(from, mess.error.Yt4, id))
                    await limitAdd(serial)
                }
            } catch (err) {
        ...
        case prefix+'play':
            ...
            try {
                axios.get(`https://turuapi.org/dl/ytplayaudio?apikey=${turukey}&query=${body.slice(6)}`).then(res => {
                    const { title, thumbnail, duration, viewCount, likeCount, dislikeCount, rating, filesize, audio, description } = res.data.result
                    tobz.sendFileFromUrl(from, thumbnail, 'yt.jpg', `*「 YOUTUBE PLAY AUDIO 」*\n\n•*Title:* ${title}\n*Duration:* ${duration}\n*likeCount:* ${likeCount}\n*dislikeCount:* ${dislikeCount}\n*Filesize:* ${filesize}\n\n_Mohon menunggu proses pengiriman audio_`)
                    tobz.sendFileFromUrl(from, audio, '', '', id)
                }).catch(e => {
                    return
                })
            } catch (err) {
	...
	case prefix+'moddroid':
        ...
        case prefix+'artinama':
            ...
            try {
            const resp = await axios.get('https://turuapi.org/tools/artinama?nama=' + body.slice(9) + '&apikey=' + turukey)
            if (resp.data.error) return tobz.reply(from, resp.data.error, id)
            const anm2 = `*「 ARTI NAMA 」*\n\n• Artinama : ${resp.data.result.arti}`
...
➸ *Total Rekaman:* ${recording}`
        ...
        case prefix+'joox':
            ...
            tobz.reply(from, mess.wait, id)
            axios.get(`https://turuapi.org/dl/joox?apikey=${turukey}&query=${body.slice(6)}`).then(result => {
                tobz.sendFileFromUrl(from, result.data.result.thumbnail, 'thumb.jpg', `*Title:* ${result.data.result.title}\n*Singer:* ${result.data.result.singer}\n*Album:* ${result.data.result.album}\n*Tgl rilis:* ${result.data.result.realease_date}\n*Duration:* ${result.data.result.duration}\n*Size:* ${result.data.result.size}\n\n*Lirik:*\n${result.data.result.lyric}\n\n_Mohon menunggu proses pengiriman audio_`)
                tobz.sendFileFromUrl(from, result.data.result.mp3, '', '', id)
            }).catch(e => {
                return
            })
            break
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'puisi2': // ARUGAZ
            ...
            break
        case 'darkjokes': // BY TURUAPI.ORG
        case 'darkjoke':
            if (isReg(obj)) return
            if (cekumur(cekage)) return
            if (isLimit(serial)) return
            axios.get(`https://turuapi.org/tools/darkjokes?apikey=${turukey}`).then(result => {
                tobz.sendFileFromUrl(from, result.data.result.joke, 'jokes.jpg', '', id)
            }).catch(e => {
                return
            })
            break
        case 'storyhorror': // BY TURUAPI.ORG
        case 'horrorstory':
        case 'horror':
            if (isReg(obj)) return
            if (cekumur(cekage)) return
            if (isLimit(serial)) return
            axios.get(`https://turuapi.org/tools/story_horror?apikey=${turukey}`).then(result => {
                tobz.reply(from, `Judul: *${result.data.result.title}*\n\n${result.data.result.story}`, id)
            }).catch(e => {
                return tobz.reply(from, 'silahkan coba lagi', id)
            })
            break
